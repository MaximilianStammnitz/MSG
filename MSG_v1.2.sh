...
genotype_calls() {
    ...
    if [ $OUTDIR/regions/nchroms = 1 ]; then
        ...
        while read chrom; do
	   vcf-concat $OUTDIR/3_genotyped/${chrom}/*.vcf.gz > $OUTDIR/4_SVs_final.vcf
	...
	while read chrom; do
	   vcf-concat $OUTDIR/4_SVs_final.vcf.gz $OUTDIR/3_genotyped/${chrom}/*.vcf.gz > $OUTDIR/4_SVs_final_n.vcf
...
hash graphtyper2.6.2 2>/dev/null || { echo -e "\nERROR: graphtyper2.6.2 command not found. Please install GraphTyper2 (https://github.com/DecodeGenetics/graphtyper) and add its directory to your PATH.\n" >&2; exit 1; }
