...
# -c: number of CPUs (processes) for Manta and Graphtyper2 (optional)
...
merge_calls() {
    ...
    # Copy, rename, gzip and index each tumour_SV.vcf file within the same folder
    ...
    # Merge SV calls with svimmer ### chromosome usage here is currently devil genome dependent! ###
...
genotype_calls() {
    ...
    # Make a genotyping input summary list for both original and additional samples
    cat $CALL $GENO >> $OUTDIR/3_genotyped/input_genotyping.txt
...
if [ "$STEP" -lt 1 ]; then
    ...
    echo -e "\n(1) RUNNING MANTA INDIVIDUALLY ON EVERY TUMOUR SAMPLE"
    individual_calling
...
if [ "$STEP" -lt 2 ]; then
    ...
    echo -e "\n(2) RUNNING SVIMMER TO MERGE TUMOUR-SPECIFIC SVS"
    merge_calls
...
if [ "$STEP" -lt 3 ]; then
    ...
    echo -e "\n(3) RUNNING GRAPHTYPER2 TO GENOTYPE SVS"
    genotype_calls
